  ...
  describe('Methods', function () {
    ...
    describe('getOutputPath()', function () {
      ...
      it('should work with a relative output path', function () {
        ...
        assert.equal(path.resolve(compiler.options.output.path, '../manifest.json'), manifest.getOutputPath());
      });

      it('should output manifest in compiler output.path by default', function () {
        const { compiler, manifest } = create(configs.hello());

        assert.equal(compiler.options.output.path, path.dirname(manifest.getOutputPath()));
      });
  ...
  describe('Options', function () {
    ...
    describe('sortManifest', function () {
      ...
      it('should turn off sorting', function () {
        ...
        assert.equal('{"d.js":"d.js","c.js":"c.js","b.js":"b.js","a.js":"a.js"}', manifest.toString());
      });
    ...
    describe('assets', function () {
      ...
      it('should set the initial assets data', function () {
        ...
        assert.equal('{"Ginger.jpg":"images/Ginger.jpg","logo.svg":"images/logo.svg"}', manifest.toString());
      });
    ...
    describe('merge', function () {
      async function setupManifest(manifest) {
        await mkdir(path.dirname(manifest.getOutputPath()));
    ...
    describe('writeToDisk', function () {
      ...
      it('Calls options.devServer.writeToDisk() with manifest path', async () => {
        const { manifest } = create(configs.devServer(__dirname), {
          writeToDisk: 'auto',
        });
  ...
  describe('Hooks', function () {
    ...
    describe('Options', function () {
      it('Options can be altered with a hook', function () {
        ...
        const { manifest } = create(configs.hello(), [undefined, manifest => manifest.hooks.options.tap('test', mock)]);
  ...
  describe('Usage with webpack', function () {
    ...
    it('should support multi compiler mode', done => {
      ...
      webpack(multiConfig, function (err) {
        ...
        assert.strictEqual(multiConfig[0].plugins[0].assets, multiConfig[1].plugins[0].assets);
